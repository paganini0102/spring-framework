...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition) throws TransactionException {
		Object transaction = doGetTransaction(); // Transaction对象的取得由具体的事务处理器实现，比如DataSourceTransactionManager
		...
		if (definition == null) {
			...
			definition = new DefaultTransactionDefinition(); // 如果没有给定任何事务处理定义就使用默认定义
		}
		// 检查当前线程是否已经存在事务，如果已经存在事务，那么需要根据在事务属性中定义的事务传播属性配置来处理事务的产生
		if (isExistingTransaction(transaction)) {
			// Existing transaction found -> check propagation behavior to find out how to behave.
			return handleExistingTransaction(definition, transaction, debugEnabled); // 对当前线程中已经有事务存在的情况进行处理，结果封装在TransactionStatus中
		}

		// Check definition settings for new transaction.
		if (definition.getTimeout() < TransactionDefinition.TIMEOUT_DEFAULT) { // 检查事务属性中timeout的设置是否合理
			throw new InvalidTimeoutException("Invalid transaction timeout", definition.getTimeout());
		}
		/**
		 * 当前没有事务存在，这时需要根据事务属性设置来创建事务
		 * 这里可以看到事务传播属性设置的处理，比如mandatory、required、required_new、nested等
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			...
			try {
				...
				doBegin(transaction, definition); // 创建事务的调用，由具体的事务处理器来完成，比如HibernateTransactionManager和DataSourceTransactionManager等
	...
	private TransactionStatus handleExistingTransaction(
		...
		// 如果当前线程已有事务存在，且当前事务的传播属性设置是never，那么抛出异常，说明这种情况是有问题的，Spring无法处理当前的事务创建
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NOT_SUPPORTED) {
			...
			boolean newSynchronization = (getTransactionSynchronization() == SYNCHRONIZATION_ALWAYS);
			/**
			 * transaction为null，newTransaction为false，意味着事务方法不需要放在事务环境中执行
			 * 意味着事务方法不需要放在事务环境中执行
		 ...
		 * 与创建全新事务的过程类似，区别在于，在创建全新事务时不用考虑已有事务的挂起，但是这里，需要考虑已有事务的挂起处理
		 */
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			...
			try {
				boolean newSynchronization = (getTransactionSynchronization() != SYNCHRONIZATION_NEVER);
				// 挂起事务的信息记录保存在TransactionStatus中，包括了进程ThreadLocal对事务信息的记录
				DefaultTransactionStatus status = newTransactionStatus(
						definition, transaction, true, newSynchronization, debugEnabled, suspendedResources);
		...
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) { // 嵌套事务的创建
		...
		// 判断在当前事务方法中的属性配置与已有事务的属性是否一致，如果不一致，那么不执行事务方法并抛出异常
	...
	protected DefaultTransactionStatus newTransactionStatus(
			...
			boolean newSynchronization, boolean debug, @Nullable Object suspendedResources) {
		/**
		 * 判断是不是新事务，如果是新事务，那么需要把事务属性存放到当前线程中
		 * TransactionSynchronizationManager维护一系列的ThreadLocal变量来保持
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				/**
				 * 把挂起事务的处理交给具体事务处理器去完成，如果具体的事务处理器不支持事务挂起，那么默认抛出异常TransactionSuspensionNotSupportedException
				 */
				if (transaction != null) {
	...
	public final void commit(TransactionStatus status) throws TransactionException {
		if (status.isCompleted()) { // 在TransactionStatus中标识事务已经结束
			throw new IllegalTransactionStateException(
					"Transaction is already completed - do not call commit or rollback more than once per transaction");
		}

		DefaultTransactionStatus defStatus = (DefaultTransactionStatus) status; // 如果事务处理过程中发生了异常，调用回滚
		...
		processCommit(defStatus); // 处理提交的入口
	}
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				prepareForCommit(status); // 事务提交的准备工作由具体的事务处理器来完成
